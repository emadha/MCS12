...
export default function ByMakeAndYearAndModel ({ title, data = [] }) {
    const { lang } = useContext(AppContext)
    const [loadedData, setLoadedData] = useState(data?.data ?? [])

    const groupedData = loadedData?.reduce((acc, item) => {
        const firstLetter = item.series[0].toUpperCase()
    ...
    const filter = (e) => {
        ...
        if (loadedData && e.target.value.trim()) {
            setLoadedData(data?.data.filter(f => f.series.toLowerCase().includes(e.target.value.toLowerCase())))
        } else {
        ...
        <div className={'flex flex-wrap'}>
            {Object.entries(groupedData).map(([letter, items]) => (
                <div key={letter} className={'w-1/6 block'}>
                    ...
                    <ul className={'p-0'}>
                        {items.map(item => (
                            <li key={item.series_slug}>
                                <Link href={route('database.cars.series', [item.make_slug, item.year, item.model_slug, item.series_slug])}
