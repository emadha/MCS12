...
export default function ByMakeAndYear ({ title, data = [] }) {
    ...
    const filter = (e) => {
        ...
        if (loadedData && e.target.value.trim()) {
            setLoadedData(data?.data.filter(f => f.model.toLowerCase().includes(e.target.value.toLowerCase())))
        } else {
    ...
    return <PageContainer title={title}>
        ...
        <h2 className={'select-none'}>Select car <strong>Model</strong></h2>
                    ...
                    <ul className={'p-0'}>
                        {items.map(item => (
                            <li key={item.model_slug} className={'list-none'}>
                                <Link href={route('database.cars.model', [item.make_slug, item.year, item.model_slug])}
