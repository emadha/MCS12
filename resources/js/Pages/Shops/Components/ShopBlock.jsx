    ...
    return loadedShop ?
        <div className={'h-full relative'}>
            <Link href={loadedShop.link}
                  className={'bg-white dark:bg-neutral-800/30 content-between flex flex-wrap h-full w-full items-center justify-start '
                ...
                <div className={'flex items-center justify-center h-full w-full'}>
                    {loadedShop.primary_photo ?
                        <img src={loadedShop.primary_photo.path.square_sm}
                             className={'object-cover w-full rounded dark:bg-neutral-900/50 block h-full border-none '} alt={' '}/>
                        : <div className={'flex items-center justify-center h-full dark:text-neutral-700 animate-pulse'}>
                            <FontAwesomeIcon icon={faCube}/>
                        </div>}
                </div>

                <div className={'absolute bottom-0 w-full flex flex-wrap gap-x-2'}>
                    <h5 className={'w-full px-3 text-3xl m-0 font-black'}>{loadedShop.title}</h5>
