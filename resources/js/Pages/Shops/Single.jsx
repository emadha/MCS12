...
export default function Single({title, shop, unique_makes = [], unique_models = []}) {
    ...
    const fetchListing = (type = 'cars') => {
        ...
        axios.get(route('api.shop.listings', {
            ...
            page: !selectedShopListingMakes.length || !selectedShopListingModels.length ? 0 : shopListingPage,
        })).then(
            r => {
            ...
            <div className={'flex flex-wrap items-start justify-center mt-10'}>
                ...
                <Field className={'w-3/4'}>
                    <div className={'p-5 pt-0 space-y-5'}>
                        ...
                        <div className={'flex items-center select-none'}>
                            <span className={'text-xs text-neutral-500'}>Available Car Models</span>
                            <div className={'flex items-center flex-wrap'}>
                                {Object.keys(shopListingModels).map((m, i) => <span
                                        key={i}
                                        onClick={(e) => {
                                            setSelectedShopListingModels([m])
                                            setSelectedShopListingMakes([])
                                            if (selectedShopListingModels.includes(m)) {
                                                setSelectedShopListingModels([])
                                            }
                                        }}
                                        className={'px-1 underline text-sm cursor-pointer hover:font-bold ' +
                    ...
                    {loadingShopListings
                        ...
                        : shopListings?.length ? <div className={'flex flex-wrap mt-10'}>

                            {shopListings.map((item, i) => <div className={'flex flex-wrap w-full'} key={i}>
                                <ListingItemCarBlock k={item.h} itemsView={'list'} item={item} className={'h-full'}/>
                            </div>)}
                        ...
                        </div> : <p className={'text-xs block text-center select-none'}>Nothing found</p>
                    }
                </Field>
            </div>
