...
export default function Error503 ({ title, refresh = null }) {
    ...
    useEffect(() => {
        ...
        refreshAfter !== null && setRefreshInterval(() => setInterval(() => {
    ...
    return <div className={''}>
        ...
        <div className={'bg-black/80 backdrop-blur-2xl fixed text-white h-screen w-screen flex items-center justify-center gap-x-5 '}>
            <div className={'flex flex-wrap items-center justify-center'}>
                <h2 className={'select-none font-bold flex w-full items-center justify-center gap-x-2'}>
                    <FontAwesomeIcon icon={faCubes} className={'opacity-50'}/>
                    {lang(title)}
                </h2>
                {refreshAfter !== null ? <small className={'opacity-50'}>{refreshAfter > 0 ? <>Refresh after {refreshAfter} seconds</> : <PrimaryButton
