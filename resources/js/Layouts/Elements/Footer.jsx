...
import { faCheck, faCookie, faDatabase, faFileContract, faHeart, faLock, faNoteSticky, faStar, faUserShield } from '@fortawesome/free-solid-svg-icons'
...
export default function Footer ({ className, children }) {
    ...
    return <>
        <Hr className={'max-w-md mx-auto !mt-20 block'} />
        <div className={'container text-sm py-10 gap-y-5 flex flex-wrap items-start justify-center'}>
            <div className={'px-5 md:w-4/6 w-full h-full text-xs'}>
                <div
                    ...
                    {settings?.locales ? <div className={'flex flex-wrap gap-x-5 items-center'}>
                        {Object.entries(settings?.locales?.list).map(language =>
                            <div className={'cursor-pointer select-none w-full xs:w-auto my-1 xs:my-0'
                    ...
                    <div
                        ...
                        {settings?.regions?.list &&
                            Object.entries(settings?.regions.list)?.
                                map((region, k) =>
                ...
                <div className={'text-neutral-500 flex flex-wrap justify-center md:justify-start gap-y-3'}>
                    <div className={'flex w-full gap-x-5 gap-y-2 flex-wrap md:justify-start justify-center'}>
                        ...
                        <AppLink href={route('pages.terms')} icon={faFileContract}>{lang('Terms and Conditions')}</AppLink>
            ...
            <div className={'w-full md:w-2/6 px-7'}>
                <div
                    ...
                    <span
                        className={'w-full text-center md:text-right md:rtl:text-left whitespace-nowrap'}>&copy; Mecarshop.com 2013 - {new Date().getFullYear()}</span> <span>{lang(
